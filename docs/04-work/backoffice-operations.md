...
- 로컬 세이브가 유일한 저장소다. Firestore 클라우드 백업을 제거해 서버 사본이 없다.
- 백오피스에서 조회하거나 복구할 세이브 문서가 없다. 세이브 문제는 사용자 기기의
  로컬 데이터 문제이므로, 재현 자료를 받아 앱 쪽에서 다룬다.
- 과거 클라우드 백업이 켜져 있던 기간에 쌓인 Firestore 문서는 앱에서 더 이상 읽지
  않는다. 그 데이터 정리는 별도 운영 작업으로 다룬다.
