...
type FarmUIState = {
  ...
  lastComboAt: number;
  /** True from HARVEST_ALL commit until DRAIN_HARVEST_ALL_EFFECT commit. Managed in the
   *  reducer so a concurrent-mode render abort cannot leave the guard permanently stuck. */
  harvestAllInFlight: boolean;
...
type FarmUIAction =
  ...
  | {
      ...
      payload: {
        effectId: number;
        /** Per-plot roll keyed by index — idempotent on StrictMode double-invoke. */
        rollFor: (plotIndex: number) => number;
        now: number;
      };
    }
  | { type: 'DRAIN_HARVEST_EFFECTS' }
  | { type: 'DRAIN_HARVEST_ALL_EFFECT' }
  | { type: 'RESET_COMBO' }
...
function farmUIReducer(state: FarmUIState, action: FarmUIAction): FarmUIState {
  switch (action.type) {
    ...
    case 'DRAIN_HARVEST_EFFECTS':
      return state.pendingHarvestEffects.length === 0 ? state : { ...state, pendingHarvestEffects: [] };

    case 'DRAIN_HARVEST_ALL_EFFECT':
      return state.pendingHarvestAllEffect == null && !state.harvestAllInFlight
        ? state
        : { ...state, harvestAllInFlight: false, pendingHarvestAllEffect: null };
    ...
    case 'HARVEST_CROP': {
      ...
      // comboCount is incremented atomically on every successful harvest, so
      // effectiveCombo is always the confirmed count — no need to add pending.length.
      const effectiveCombo = state.comboCount;
...
export default function FarmGame({
  ...
  // computes. comboCount is incremented atomically with each HARVEST_CROP dispatch,
  // so the display is always in sync with the actual committed harvest count.
  const comboDisplay = farmState.comboCount;
  ...
  // Process a committed harvestAll effect from the reducer. try/finally ensures
  // DRAIN_HARVEST_ALL_EFFECT (which also clears harvestAllInFlight) always fires,
  // even if a feedback call throws.
  useEffect(() => {
  ...
  function harvestAllCrops() {
    ...
    const now = Date.now();
    const effectId = ++commandEffectIdRef.current;
    // One stable roll per plot index — not a flat sequence. Built here so a
    // StrictMode double-invoke of the reducer reuses the same closure and returns
    // identical rolls (rollByPlot is already populated on the second call).
