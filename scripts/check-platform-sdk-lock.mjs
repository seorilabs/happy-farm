...
export function checkPlatformSdkLock(root, approvedVersion = APPROVED_SDK_VERSION) {
  ...
  const foreignLocks = paths.filter((path) => FOREIGN_LOCKFILES.includes(basename(path)));
  for (const path of foreignLocks) {
    problems.push(`${path}: 이 저장소는 pnpm으로 설치한다. 다른 package manager의 lockfile을 커밋하면 package manager 신호가 갈려 Platform discovery가 SDK 탑재를 확인하지 못한다.`);
  }
