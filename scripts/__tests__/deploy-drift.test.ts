...
describe('renderDriftTable', () => {
  test('step summary 표에 최신 태그·채널 행·드리프트 상태가 포함된다', () => {
    const rows = [
      {
        channel: 'AIT (WEB)',
        deployedTag: 'v1.7.0',
...
describe('pickNewerRun', () => {
  test('created_at이 더 최근인 run을 고른다 (단독 run vs deploy-all 경유)', () => {
...
// 수집한 배포 run + 태그로 채널 판정 행을 만드는 로직을 검증(통합 검증은 deploy-drift-run.test.ts).
describe('buildChannelRow (#420 채널 수집·비교)', () => {
  const referenceMs = Date.parse('2026-07-23T00:00:00Z'); // 최신 태그 v1.8.3 시각 가정

  test('수집한 단독/deploy-all run과 태그로 WEB(AIT) v1.7.0 정체를 드리프트로 판정한다 (#420 재현)', () => {
...
// 드리프트 감지 시 발행하는 이슈/코멘트 본문이 마커와 표를 담는지 검증(통합 검증은 deploy-drift-run.test.ts).
describe('buildIssueBody', () => {
  test('드리프트 이슈/코멘트 본문에 마커·채널 비교 표·점검 메타가 포함된다', () => {
    const rows = [
      {
        channel: 'AIT (WEB)',
        deployedTag: 'v1.7.0',
...
describe('chooseIssueAction', () => {
  test('마커가 있는 열린 이슈가 있으면 코멘트, 없으면 생성을 택한다', () => {
