...
// #362: selectSeed 온보딩 병목 방어를 위한 순수 결정 로직.
// 실제 애니메이션/렌더는 FarmGame이 담당하고, "언제/어디로 유도할지"의 판정만
// 여기서 분리해 헤드리스 단위 테스트로 검증한다.

// AC-3: stall 시각 넛지는 selectSeed 단계에서만, 그리고 세션당 1회만 발화한다.
// alreadyFired는 세션 스코프의 1회 가드 상태.
export function shouldFireStallNudge(step: OnboardingStep | null, alreadyFired: boolean): boolean {
  return step === 'selectSeed' && !alreadyFired;
}

// AC-2: 바로 시작 CTA는 대표 씨앗(quickStartCropKey)이 존재할 때 렌더된다.
// getOnboardingCropKey가 항상 non-null을 보장하므로 selectSeed 단계에서 CTA는
...
// AC-4: selectSeed에서 감당 불가 작물을 탭했을 때 시선을 유도할 대상 씨앗.
// 감당 가능한 대표 씨앗을 돌려주고(그 작물의 구역으로 스크롤/전환해 노출),
// selectSeed가 아니거나 탭한 작물이 애초에 감당 가능하거나 감당 가능한 대표
